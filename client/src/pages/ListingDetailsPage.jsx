...
import CallButton from "../components/CallButton"; // ✅ In-app voice call
...
const ListingDetailsPage = () => {
    ...
    const handleTourSubmit = async (e) => {
        ...
        try {
            const response = await fetch(`${import.meta.env.VITE_API_URL || "http://localhost:3000"}/api/listings/${id}/tour`, {
                method: "POST",
                headers: {
    ...
    useEffect(() => {
        // ✅ Read from THIS TAB's sessionStorage (per-tab isolation)
        const sessionInfo = sessionStorage.getItem("sessionUserInfo");
        const userInfo = sessionInfo || localStorage.getItem("userInfo");
        if (userInfo) {
        ...
        const fetchListing = async () => {
            try {
                const response = await fetch(`${import.meta.env.VITE_API_URL || "http://localhost:3000"}/api/listings/${id}`);
                const data = await response.json();
                if (response.ok) {
                    setListing(data);
                } else {
    ...
    const handleFavorite = async () => {
        ...
        try {
            const response = await fetch(`${import.meta.env.VITE_API_URL || "http://localhost:3000"}/api/auth/favorites/${id}`, {
                method: method,
                headers: {
                    Authorization: `Bearer ${token}`,
                },
                            ...
                            <Card.Body className="p-4">
                                ...
                                {!isOwnerOrAdmin && (
                                    <div className="d-grid gap-3">
                                        <Button variant="primary" size="lg" className="fw-bold py-3" onClick={() => {
                                            if (!currentUser) {
                                                setIsLoginView(true);
                                        ...
                                        {/* ✅ In-app voice call — no phone numbers shared */}
                                        {currentUser && listing.user?._id && (
                                            <CallButton
                                                listingOwnerId={listing.user._id}
                                                listingOwnerName={listing.user.name || "Owner"}
                                            />
                                        )}
                                            ...
                                            <img
                                                src={
                                                    listing.user.profilePicture.startsWith("http")
                                                        ? listing.user.profilePicture
                                                        : `${import.meta.env.VITE_API_URL || "http://localhost:3000"}${listing.user.profilePicture}`
                                                }
