...
const ProfilePage = () => {
    ...
    const fetchUserProfile = async () => {
        // Use THIS TAB'S token from sessionStorage
        ...
        try {
            ...
            if (response.ok) {
                setUser(data);
                // Update both storages consistently for this tab
    ...
    useEffect(() => {
        ...
        // This prevents cross-tab bleed: Vikash's tab always shows Vikash, etc.
        ...
        if (userInfo) {
            ...
            fetchMyListings();
            fetchUserProfile(); // fetches fresh data from API using this tab's token
        } else {
            navigate("/");
        }
    }, [navigate]);
    ...
    const handleCropSave = async () => {
        ...
        try {
            ...
            // Upload using FormData (multer on server expects multipart/form-data with field "image")
            const formData = new FormData();
            formData.append("image", file);

            const token = sessionStorage.getItem("sessionToken") || localStorage.getItem("token");
            const uploadResp = await fetch(`${import.meta.env.VITE_API_URL || ''}/api/upload`, {
                method: 'POST',
            ...
            const uploadedPath = await uploadResp.text(); // server returns plain string like "/uploads/image-xxx.jpg"

            setProfilePicture(uploadedPath);
            setUploading(false);
    ...
    const handleUpdateProfile = async (e) => {
        ...
        try {
            ...
            if (response.ok) {
                // Update both storages so this tab stays consistent
                sessionStorage.setItem("sessionUserInfo", JSON.stringify(data));
                localStorage.setItem("userInfo", JSON.stringify(data));
                if (data.token) {
